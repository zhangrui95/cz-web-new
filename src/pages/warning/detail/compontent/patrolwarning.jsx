				...
				if (files[i].label_gps_point) {
                    ...
                    if(files[i].label_type == 2){
                        ...
                        polygonFeature = new ol.Feature({
                            geometry: new ol.geom.Polygon(this.transform(files[i].label_gps_point[0],files[i].label_gps_point[1] ))
                        })
                        ...
                        //根据gps点构建休息点
                        polygonFeature = new ol.Feature({
                            geometry: new ol.geom.Point(this.transform(files[i].label_gps_point[0],files[i].label_gps_point[1] ))
                        })
                        polygonFeature.setStyle(
                            new ol.style.Style({
                        ...
                        //根据gps点构建网格
                        polygonFeature = new ol.Feature({
                            geometry: new ol.geom.LineString(this.transform(files[i].label_gps_point[0],files[i].label_gps_point[1] ))
                        })
                        polygonFeature.setStyle(
                            new ol.style.Style({
                                stroke: new ol.style.Stroke({
                    ...
                    //根据gps点构建当前点
                        ...
                        brr.push(this.transform(files[i].label_gps_point[0],files[i].label_gps_point[1] ))
                        polygonFeature.setStyle(
                            new ol.style.Style({
                                image: new ol.style.Icon({
                                    scale: 1.1, // 图标缩放比例 // 0为离线其他为在线

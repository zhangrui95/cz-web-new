...
class patrol extends React.Component {
	...
	getTableData = (changePage, pd) => {
		...
		const { dispatch, scouts: { patrol: { page } } } = this.props
		...
		dispatch({
			type:'scouts/getOrgPatrolStatisticsList',
			payload: param
		})
	}

	onChange = (currentPage) => {
		const { scouts: { patrol: { page } } } = this.props
		...
		return (
			<Form layout="inline" onSubmit={this.handleSubmit} {...formItemLayout}>
				<Row>
					<Col span={8}>
						<FormItem label="机构名称">
							{getFieldDecorator('schedule_organization_code')(
								<TreeSelect
...
//导出
		...
		form.validateFields((err, fieldsValue) => {
			...
			window.open(
				`${'./dow.html?serverUrl='}${window.configUrl
					.serverUrl}${'&fieldsValue='}${fieldsValues}${'&currentXLSX='}${'statistics/exportOrgPatrolStatisticsList'}`,
				'_blank'
			)
		})
	}
	// 查询条件重置
	handleFormReset = () => {
		const { form, scouts: { patrol: { page } } } = this.props
		form.resetFields()
		this.setState(
			{
				formValues: {},
	...
	render() {
		const { scouts: { patrol: { list, page } } } = this.props
		...
		const columns = [
			...
			{
				title: '机构名称',
				dataIndex: 'schedule_organization_name',
				ellipsis: true
				// width: 120
			},
			{

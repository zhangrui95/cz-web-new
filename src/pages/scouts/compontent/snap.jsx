...
class snap extends React.Component {
	...
	getTableData = (changePage, pd) => {
		...
		const { dispatch, scouts: { snap: { page } } } = this.props
	...
	getInterface = () => {
		...
		return muneKey == '0' ? 'scouts/getOrgZPStatisticsList' : 'scouts/getDateZPStatisticsList'
	}
	...
	currentXLSX() {
		const { muneKey } = this.state
		return muneKey == '0' ? 'statistics/exportOrgZPStatisticsList' : 'statistics/exportDateZPStatisticsList'
	}
	// 查询条件重置
	handleFormReset = () => {
		const { form, scouts: { snap: { page } } } = this.props
		form.resetFields()
		this.setState(
			{
				formValues: {},
	...
	render() {
		const { scouts: { snap: { list, page } } } = this.props
		const { muneKey } = this.state
		const munes = [
			{
				key: '0',
				name: '机构',
                permissions: 'czht_xfzgtj_zptj_jg'
			},
			{
				key: '1',
				name: '时间',
                permissions: 'czht_xfzgtj_zptj_sj'
			}
		]
		const columns = [
			{
...
export default Form.create()(snap)

...
class statistical extends React.Component {
    ...
    componentDidMount() { }
    render() {
        const { otherIndex, vehicleStatusList, schedules, vehicleState, vehicleDetailInfor } = this.props;
        const { isCar } = this.state
        const user = sessionStorage.getItem('user') ? JSON.parse(sessionStorage.getItem('user')) : ''
        let carNum = window.carNum ? window.carNum(
            JSON.parse(sessionStorage.getItem('user'))
                ? JSON.parse(sessionStorage.getItem('user')).department
                : '',
        ) : vehicleStatusList&&vehicleStatusList.length&&vehicleStatusList[0].online ? vehicleStatusList[0].online : '0';
        let carNumOff = window.carNum ? 0 : vehicleStatusList&&vehicleStatusList.length&&vehicleStatusList[0].offline ? vehicleStatusList[0].offline : '0'
        return (
            <div className={styles.statistical}>
                <div className={styles.classificationTitle} style={{backgroundImage: "url('./image/bt_bj.png')" }}>{!vehicleState ? `${user != '' ? user.group.name : ''}` : `警车  ${vehicleDetailInfor.carNo}`}</div>
                {
                    ...
                    <div className={styles.attendanceCar}>
                        <div className={styles.item}>
                            ...
                            {/*<span className={styles.itemCount}>{vehicleStatusList&&vehicleStatusList.length&&vehicleStatusList[0].online ? Number(vehicleStatusList[0].online) + Number(vehicleStatusList[0].offline ? vehicleStatusList[0].offline : 0): vehicleStatusList&&vehicleStatusList.length&&vehicleStatusList[0].offline ? vehicleStatusList[0].offline : '0'}</span>*/}
                            <span className={styles.itemCount}>{carNum}</span>
                        </div>
                        <div className={styles.item}>
                            ...
                            <span className={styles.itemCount}>{carNumOff}</span>
                        </div>
                    </div>
                    :
                ...
                <div className={styles.attendancePersonnel}>
                    ...
                    <div className={styles.item}>
                    {
                        ...
                        <p className={`${styles.itemTitle} ${styles.lack}`}>已出勤人数</p>
                    </div>
                </div>
            </div>
        );
